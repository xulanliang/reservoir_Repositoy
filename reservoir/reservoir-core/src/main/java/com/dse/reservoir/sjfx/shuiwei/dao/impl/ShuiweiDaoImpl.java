 ...
 * 水位Dao层
 */
@Repository("shuiweiDao")
public class ShuiweiDaoImpl extends BaseDao implements ShuiweiDao {

    @Override
    public EasyUiPage getDateList(String stm, String etm, GcxEntity gcxArgs , EasyUiPage easyUiPage) throws Exception {
        List<Object> params = new ArrayList<Object>();
        String sql = "SELECT A.STCD,S.STNM,TO_CHAR(A.TM,'yyyy-mm-dd hh24:mi:ss') AS TM,round(Z,2) Z FROM WR_SW_R A LEFT JOIN WR_ST_PROJ_TYPE_MAP M ON A.STCD = M.STCD " +
    ...
    public Object getGcxDateList(String stm, String etm, GcxEntity gcxArgs) throws Exception{
        ...
        String sql = "SELECT A.STCD,S.STNM,TO_CHAR(A.TM,'yyyy-mm-dd') AS TM,round(Z,2) DATA FROM WR_SW_R A LEFT JOIN WR_ST_PROJ_TYPE_MAP M ON A.STCD = M.STCD " +
                "LEFT JOIN WR_STAT_M S ON A.STCD = S.STCD WHERE M.ST_TP = '5' AND S.ISJIDIAN = '0' ";
    ...
    public EasyUiPage getGcjlbList(String tm, String ppcd, String projcd, EasyUiPage easyUiPage) throws Exception {
        if(StringUtil.isNotEmpty(projcd)) {
            String sql2 = "SELECT STCD FROM WR_ST_PROJ_TYPE_MAP WHERE ST_TP = '5' AND PROJ_CD = ? ";
