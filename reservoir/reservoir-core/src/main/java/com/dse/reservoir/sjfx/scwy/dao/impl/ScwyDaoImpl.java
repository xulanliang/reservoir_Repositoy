package com.dse.reservoir.sjfx.scwy.dao.impl;
 ...
 */
@Repository("scwyDao")
public class ScwyDaoImpl extends BaseDao implements ScwyDao {

    @Override
    public EasyUiPage getDateList(String stm, String etm, GcxEntity gcxArgs, EasyUiPage easyUiPage) throws Exception {
        List<Object> params = new ArrayList<Object>();
        String sql = "SELECT A.STCD,S.STNM,TO_CHAR(A.TM,'yyyy-mm-dd hh24:mi:ss') AS TM,round(NVL(A.CALCDATA,A.ORIGDATA),2) AS DATA FROM WR_MSFT_B A LEFT JOIN WR_ST_PROJ_TYPE_MAP M ON A.STCD = M.STCD " +
                "LEFT JOIN WR_STAT_M S ON A.STCD = S.STCD WHERE M.ST_TP = '8' AND S.ISJIDIAN = '0' ";
    ...
    public Object getGcxDateList(String stm, String etm, GcxEntity gcxArgs) throws Exception{
        ...
        String sql = "SELECT A.STCD,S.STNM,TO_CHAR(A.TM,'yyyy-mm-dd') AS TM,round(NVL(A.CALCDATA,A.ORIGDATA),2) AS DATA FROM WR_MSFT_B A LEFT JOIN WR_ST_PROJ_TYPE_MAP M ON A.STCD = M.STCD " +
                "LEFT JOIN WR_STAT_M S ON A.STCD = S.STCD WHERE M.ST_TP = '8' AND S.ISJIDIAN = '0' ";
    ...
    public EasyUiPage getGcjlbList(String tm, String ppcd, String projcd, EasyUiPage easyUiPage) throws Exception {
        if(StringUtil.isNotEmpty(projcd)) {
            String sql2 = "SELECT STCD FROM WR_ST_PROJ_TYPE_MAP WHERE ST_TP = '8' AND PROJ_CD = ? ";
