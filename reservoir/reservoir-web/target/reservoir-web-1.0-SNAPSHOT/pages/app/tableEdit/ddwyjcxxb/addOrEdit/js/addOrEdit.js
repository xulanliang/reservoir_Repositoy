...
define('addOrEdit', function(require, exports, module) {
		...
		methods: {
			...
			doSave: function() {
				...
				if(this.flag) {
					...
					//执行保存方法
					$ajax({
						url: DSE.serverPath + 'tableEditCtrl/saveOrUpdateDdwyjcxxInfo.do',
						async: true,
						data: {
	...
	$("#PROJ_CD_ONE_auto").combobox({
		...
		onChange: function(val){
			...
			var vv = getDataInfoExtend(DSE.serverPath + '/initBaseInfoCtel/initSectionExtendInfo.do',levOneID)["data"] || [];
			$("#PROJ_CD_TWO_auto").combobox("loadData",vv);
			$("#ITEM_CD_auto").combobox("loadData",vv);
			var datVal =$("#PROJ_CD_TWO_auto").combobox("getValue");
			var datVal2 =$("#PROJ_CD_ONE_auto").combobox("getValue");
			//回显二级工程名称
			for(var i=0;i<vv.length;i++){
				if(datVal == vv[i].PROJ_NM){
					$("#PROJ_CD_TWO_auto").combobox("select",vv[i].PROJ_CD_TWO);
					return;
				}
			}
			//回显一级工程名称
			for(var i=0;i<data.length;i++){
				if(datVal2 == data[i].PROJ_NM){
					$("#PROJ_CD_TWO_auto").combobox("select",data[i].PROJ_CD_TWO);
					return;
				}
			}
			//$("#PROJ_CD_TWO_auto").combobox("setValue","左副坝");
		}
	});
	...
	if(app.operType == 'add'){
        ...
        //同步加载请求
        var data = $ajax({
        	url: DSE.serverPath + 'tableEditCtrl/getDdwyjcxxlnfoById.do?id=' + app.rowId
        }) || [];
