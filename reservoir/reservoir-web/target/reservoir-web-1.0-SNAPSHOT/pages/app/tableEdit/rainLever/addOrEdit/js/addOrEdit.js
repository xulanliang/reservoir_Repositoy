...
define('addOrEdit', function(require, exports, module) {
		...
		methods: {
			...
			doSave: function() {
				...
				if(data["CKNAME"] == '') {
					...
					//执行保存方法
					$ajax({
						url: DSE.serverPath + '/tableEditCtrl/saveOrUpdateInfo.do',
						async: true,
						data: {
	...
	if(app.operType == 'add'){
        ...
        //同步加载请求
        var data = $ajax({
        	url: DSE.serverPath + '/tableEditCtrl/getInfoById.do?id=' + app.rowId + '&tm=' + app.tm
        }) || [];
        
        //填充数据到表单元素,easyui.dseForm.js里的方法
		var str = data.data[0].TM;
		data.data[0].TM = parent.getTime(parseInt(str));
