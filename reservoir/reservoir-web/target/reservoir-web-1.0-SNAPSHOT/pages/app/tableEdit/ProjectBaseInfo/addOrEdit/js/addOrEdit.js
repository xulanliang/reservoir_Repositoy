...
define('addOrEdit', function(require, exports, module) {
		...
		methods: {
			...
			doSave: function() {
				...
				if(data["CKNAME"] == '') {
					...
					//执行保存方法
					$ajax({
						url: DSE.serverPath + '/tableEditCtrl/saveOrUpdateProjectBaseInfo.do',
						async: true,
						data: {
	...
	if(app.operType == 'add'){
        ...
        //同步加载请求
        var data = $ajax({
        	url: DSE.serverPath + '/tableEditCtrl/getProjectBaseInfoById.do?id=' + app.rowId
        }) || [];
        
        //填充数据到表单元素,easyui.dseForm.js里的方法

		 var dt = data.data[0]['TS'];
		data.data[0]['TS']=parent.getDate(dt);
