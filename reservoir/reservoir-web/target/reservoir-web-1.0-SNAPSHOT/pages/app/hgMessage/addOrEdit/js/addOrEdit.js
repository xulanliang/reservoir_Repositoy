...
define('addOrEdit', function(require, exports, module) {
		...
		methods: {
			...
			doSave: function() {
				...
				//} else if(this.flag) {
					...
					//执行保存方法
					$ajax({
						url: DSE.serverPath + '/cdgcController/saveOrUpdateHgList.do',
						async: true,
						data: {
	...
	if(app.operType == 'add'){
        ...
        //同步加载请求
        var data = $ajax({
        	url: DSE.serverPath + '/cdgcController/getHgList.do?id=' + app.rowId
        }) || [];

        //填充数据到表单元素,easyui.dseForm.js里的方法

		var ST_TP=data.rows[0].ST_TP;
		console.log(data.rows[0].ST_TP);
