...
require_css('css!pages/app/sjfx/szsk/shenLiu/gcjlb/css/gcjlbPage.css');

require_js('gcjlb_search', 'pages/app/sjfx/szsk/shenLiu/gcjlb/js/gcjlbsearch', ['eform']);
require_js('gcjlb_datagrid', 'pages/app/sjfx/szsk/shenLiu/gcjlb/js/gcjlbdatagrid', ['gcjlb_search', 'datagrid_pager']);
    ...
    DSE.app = new Vue({
        ...
        methods: {
            ...
            doExport: function() {
                ...
                var url= DSE.serverPath + '/shenLiuCtrl/exportSZSKGcjlbExcel.do?' +searchData + '&rows='+rows + '&fileName='+encodeURIComponent(encodeURIComponent(fileName));
                window.location.href = url ;
                /*var exportType = '渗流-观测记录表';
                $.ajax({
                    url: DSE.serverPath + '/shenLiuCtrl/getGcjlbExcel.do',
                    async: false,
                    data: {
                    ...
                    success: function(data) {
                        $.ajax({
                            ...
                            type: 'post',
                            success: function(data) {
                                //window.location.href = decodeURI(DSE.serverPath + '/excelController/downExcel.do?exportId=' + data.data + '&shortName=渗流-观测记录表.xlsx');
                                var shortName='渗流-观测记录表.xlsx';
