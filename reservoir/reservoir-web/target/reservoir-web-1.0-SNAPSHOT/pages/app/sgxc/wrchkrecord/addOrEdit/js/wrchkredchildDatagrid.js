        ...
        $.ajax({
            ...
            success: function(data) {
                $tips.suc('操作成功', function() {
                    ...
                    // }
                    $(DSE.form.datagrid).datagrid('reload');
                });
            }
        });
    }
...
function deleteInfoByIds (ids) {
    if (nullToSpace(ids) == '') {
        ...
        $.ajax({
            url: DSE.serverPath + '/wrchkredchildCtrl/deleteInfoByIds.do',
            async: true,
            type: 'post',
            data:{
                ids:ids,
...
DSE.form.initDatagrid = function(params, _data) {
    ...
    var datagridOptions = {
        ...
        onLoadSuccess:function(data){
            log(app.operType);
            if(parent.DSE.app.operType=='detail'){
                //影藏操作栏
                $('#datagrid_wrchkredchild').datagrid('hideColumn', 'operate');
            }
        },
        columns: [
            [{
                ...
                formatter: function (value, rowData, rowIndex) {
                    ...
                    op += '<a href="javascript:" class="f-mr10" onclick="deleteInfoByIds('+ rowData[_keyId]+')"><i class="fa fa-trash-o"></i> 删除</a>';
                    op += '</div>';
                    return op;
                }
            }
