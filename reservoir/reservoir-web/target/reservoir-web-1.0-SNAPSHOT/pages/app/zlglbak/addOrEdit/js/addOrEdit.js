...
define('addOrEdit', function(require, exports, module) {
		...
		methods: {
			...
			doSave: function() {
				...
				if (data["INFO_CD"] == '') {
					...
					$tr.each(function(i){
						_newData[i] = {
							// "TM" : $tr.eq(i).find('div[absdv="TM"]').text(),
							// "DTYPE" : $tr.eq(i).find('div[data-id="DTYPE"]').attr('data-dtype'),
							// "DBLD" : $tr.eq(i).find('input[data-id="DBLD"]').combobox("getValue"),
							// "ZB" : $tr.eq(i).find('input[data-id="ZB"]').combobox("getValue"),
							// "FB" : $tr.eq(i).find('input[data-id="FB"]').combobox("getValue"),
							// "SJ" : $tr.eq(i).find('input[data-id="SJ"]').val(),
							// "JPRR" : $tr.eq(i).find('input[data-id="JPRR"]').val(),
	...
	$(adcd).combotree({
		...
		prompt: '-请选择-',
		onLoadSuccess: function () {
			 treeSetDefVal(parent.DSE.app.adcd);
		},
