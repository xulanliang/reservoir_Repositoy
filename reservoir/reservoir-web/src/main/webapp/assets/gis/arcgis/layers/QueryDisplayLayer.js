...
define([
    ...
    var moClass = declare([GraphicsLayer], {
                        ...
                        if (data.hasOwnProperty(_self.lonField) && data.hasOwnProperty(_self.latField)) {
                            ...
                            if (latVal && lonVal) {
                                ...
                                if (_self.labels) {
                                    ...
                                    if (_self.visible) {
                                        //显示标签.
                                        var labelConfig = _self.labels;
                                        if (typeof _self.labelsCallBack === "function") {
                                            //
                                            labelConfig = _self.labelsCallBack(data);
                                        }
                                        var fields = labelConfig.fields || [];
                                        //
                                        fields.forEach(function (field, index) {
                                            ...
                                            for (var key in field) {
                                                ...
                                                var content = key + ":" + data[v];
                                                //
                                                var labelSymbol = dseSymbolUtils.createTextSymbol({
                                                    //
                                                ...
                                                //
                                                var labelGra = graphicUtils.createPoint2(data, dx, dy, labelSymbol);
                                                _self.add(labelGra);
                                            }
                                        });
                                    }
        ...
        /** load polyline data .*/
        loadPolylineData: function (data, isNewData) {
            //
            if (isNewData) {
                //
                // this._executeQuery(data);
