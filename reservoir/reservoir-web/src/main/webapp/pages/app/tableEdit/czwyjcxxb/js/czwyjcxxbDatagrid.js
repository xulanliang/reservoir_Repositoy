...
DSE.form.initDatagrid =function initDatagrid(params, _data) {
    ...
    var _keyId = 'STCD', //主键ID,根据业务修改,删除也是用的此字段
        tm='TM',
        _url = DSE.serverPath + 'tableEditCtrl/getCzwyjcxxListInfoByParams.do', //数据请求地址

    //可以删除或注释
    //因为模拟数据时,分页处理时需要用到初始所有数据,所以需要定义变量存储初始数据
        _oldData = _data,
        _h = $(document).height() - 42, //计算列表的高度,根据页面内容调整此处数值
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, rowData, rowIndex) {
                    ...
                    op += '<a href="javascript:" class="f-mr10" onclick="DSE.form.showEditOrDetail(\'' + rowData["STCD"]+","+getTime(rowData["TM"])+","+rowData["QS"] + '\',\'编辑垂直位移监测信息\',\'edit\',\'./addOrEdit/addOrEdit.html\')"><i class="fa fa-edit"></i> 编辑</a>';
                    op += '<a href="javascript:" onclick="DSE.form.deleteDatagridRow(\'所选数据\', \'tableEditCtrl/deleteCzwyjcxxByIds.do\', {status: false,ids:\'' + rowData[_keyId]+","+getTime(rowData["TM"])+","+rowData["QS"]+'\'})"><i class="fa fa-trash-o"></i> 删除</a>';
                    op += '</div>';
                    return op;
                }
            }
