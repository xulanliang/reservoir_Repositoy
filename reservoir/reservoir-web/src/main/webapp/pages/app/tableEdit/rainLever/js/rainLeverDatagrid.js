...
DSE.form.initDatagrid =function initDatagrid(params, _data) {
    ...
    var _keyId = 'STCD', //主键ID,根据业务修改,删除也是用的此字段
        tm='TM',
        _url = DSE.serverPath + '/tableEditCtrl/getRainListInfoByParams.do', //数据请求地址
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, rowData, rowIndex) {
                    ...
                    var t =getTime(rowData[tm])
                    op += '<a href="javascript:" class="f-mr10" onclick="showEditOrDetailExtend(\'' + rowData[_keyId] + '\',\'' + rowData[tm] + '\',\'编辑降雨量信息\',\'edit\',\'./addOrEdit/addOrEdit.html\')"><i class="fa fa-edit"></i> 编辑</a>';
                    op += '<a href="javascript:" onclick="DSE.form.deleteDatagridRow(\'所选数据\', \'wzcfdxxCtrl/deleteInfoByIds.do\', {status: false,ids:\'' + rowData[_keyId]+','+rowData[tm] + '\'})"><i class="fa fa-trash-o"></i> 删除</a>';
                    op += '</div>';
                    return op;
                }
            }
