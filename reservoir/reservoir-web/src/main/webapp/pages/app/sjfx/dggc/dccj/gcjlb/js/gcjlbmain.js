...
require_css('css!pages/app/sjfx/dggc/dccj/gcjlb/css/gcjlbPage.css');

require_js('gcjlb_search', 'pages/app/sjfx/dggc/dccj/gcjlb/js/gcjlbsearch', ['eform']);
require_js('gcjlb_datagrid', 'pages/app/sjfx/dggc/dccj/gcjlb/js/gcjlbdatagrid', ['gcjlb_search', 'datagrid_pager']);
    ...
    DSE.app = new Vue({
        ...
        methods: {
            ...
            doExport: function() {
                ...
                var url= DSE.serverPath + '/dccjCtrl/exportGcjlbExcel.do?' +searchData+'&pointType=9' + '&rows='+rows + '&fileName='+encodeURIComponent(encodeURIComponent(fileName));
                window.location.href = url ;


                /*var exportType = '渡槽沉降-观测记录表';
                $.ajax({
                    url: DSE.serverPath + '/dccjCtrl/getGcjlbExcel.do',
                    async: false,
                    data: {
                        year: $('#year_auto').combobox('getValue'),
                        month: $('#month_auto').combobox('getValue'),
                        projcd: $('#projcd_auto').val(),                                  //'3',
                        PROJ_CD: $('#PROJ_CD_auto').combobox('getValue')                  //'3001'
                    },
                    type: 'post',
                    success: function(data) {
                        $.ajax({
                            ...
                            type: 'post',
                            success: function(data) {
                                // window.location.href = decodeURI(DSE.serverPath + '/excelController/downExcel.do?exportId=' + data.data + '&shortName=渡槽沉降.xlsx');
                                var shortName='渡槽沉降.xlsx';
