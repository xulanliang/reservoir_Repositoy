...
require_css('css!pages/app/sjfx/dggc/export/allDatas/gcjlb/css/gcjlbPage.css');

require_js('gcjlb_search', 'pages/app/sjfx/dggc/export/allDatas/gcjlb/js/gcjlbsearch', ['eform']);
require_js('gcjlb_datagrid', 'pages/app/sjfx/dggc/export/allDatas/gcjlb/js/gcjlbdatagrid', ['gcjlb_search', 'datagrid_pager']);
    ...
    DSE.app = new Vue({
        ...
        methods: {
            ...
            doExport: function() {
                ...
                var url= DSE.serverPath + '/dggcAllDataCtr/exportGcjlbExcel.do?' +searchData+'&pointType=1' + '&rows='+rows + '&fileName='+encodeURIComponent(encodeURIComponent(fileName));
                window.location.href = url ;
                /*var exportType = '水位-观测记录表';
                $.ajax({
                    url: DSE.serverPath + '/shuiweiCtrl/getGcjlbExcel.do',
                    async: false,
                    data: {
                    ...
                    success: function(data) {
                        $.ajax({
                            ...
                            type: 'post',
                            success: function(data) {
                               //window.location.href = decodeURI(DSE.serverPath + '/excelController/downExcel.do?exportId=' + data.data + '&shortName=水位-观测记录表.xlsx');
                                var shortName='水位-观测记录表.xlsx';
