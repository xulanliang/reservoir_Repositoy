...
require_js('jnbb_search', 'pages/app/sjfx/szsk/shenLiu/jnbb/js/jnbbsearch', ['eform']);
require_js('jnbb_datagrid', 'pages/app/sjfx/szsk/shenLiu/jnbb/js/jnbbdatagrid', ['jnbb_search', 'datagrid_pager']);
    ...
    DSE.app = new Vue({
        ...
        methods: {
            ...
            doExport: function () {
                ...
                var url= DSE.serverPath + '/shenLiuCtrl/exportSZSKJnbbExcel.do?' +searchData + '&rows='+rows  + '&fileName='+encodeURIComponent(encodeURIComponent(fileName));
                window.location.href = url ;

                /*var exportType = '渗流-季年报表';
                $.ajax({
                    url: DSE.serverPath + '/shenLiuCtrl/getjnbbList.do',
                    async: false,
                    data: {
                    ...
                    success: function (data) {
                        $.ajax({
                            ...
                            type: 'post',
                            success: function (data) {
                                //window.location.href = decodeURI(DSE.serverPath + '/excelController/downExcel.do?exportId=' + data.data + '&shortName=渗流-季年报表.xlsx');
                                var shortName='渗流-季年报表.xlsx';
