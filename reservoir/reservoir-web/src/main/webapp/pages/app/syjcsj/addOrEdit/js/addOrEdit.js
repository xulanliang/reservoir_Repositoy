...
define('addOrEdit', function(require, exports, module) {
		...
		methods: {
			...
			doSave: function() {
				...
				if(data["CKNAME"] == '') {
					...
					//执行保存方法
					$ajax({
						url: DSE.serverPath + '/cdgcController/saveOrUpdateSyjcList.do',
						async: true,
						data: {
	...
	if(app.operType == 'add'){
        ...
        //同步加载请求
        var data = $ajax({
        	url: DSE.serverPath + '/cdgcController/getSyjcList.do?id=' + app.rowId
        }) || [];
        //填充数据到表单元素,easyui.dseForm.js里的方法
		DSE.form.showFormContent(data.rows[0], app.operType);
		$("#TM_auto").val(changeTime(data.rows[0].TM))
	}
	//隐藏加载图片
