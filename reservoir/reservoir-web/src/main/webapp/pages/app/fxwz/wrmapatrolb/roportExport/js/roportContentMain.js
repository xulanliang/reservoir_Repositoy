...
require_css('css!pages/app/fxwz/wrmapatrolb/roportExport/tableEdit.css');

require_js('wrchkredchildSearch', 'pages/app/fxwz/wrmapatrolb/roportExport/js/roportContentSearch', ['eform']);
require_js('wrchkredchildDatagrid', 'pages/app/fxwz/wrmapatrolb/roportExport/js/roportContentDatagrid', ['wrchkredchildSearch', 'datagrid_pager']);
    ...
    DSE.app = new Vue({
        data: function () {
            return {
                ...
                isflood:parent.DSE.app.isflood,  //0:日常  1、2：汛前汛后
            }
        },
        methods: {
            ...
            doExport: function() {
                ...
                if(DSE.app.isflood == 0){
                    var fileName =  "水库-泵站防汛物资巡查记录表.xls" ;
                    //var fileName =  "水库.xls" ;
                }else {
                    var fileName =  "水库-泵站汛前及汛后防汛物资检查及养护记录表.xls" ;
                    //var fileName =  "水库大坝.xls" ;
                }
                var url= DSE.serverPath + '/wrmapatrolbCtrl/export2Excel.do?' +searchData + '&rows='+rows +'&isflood='+DSE.app.isflood+'&fileName='+ encodeURIComponent(encodeURIComponent(fileName));
            ...
            doSave: function () {
                ...
                if(rowIndex==0){
                    ...
                    // $(DSE.form.datagrid).datagrid('onClickRow', lastIndex);
                }else{
                    $('#datagrid').datagrid('selectRow',0);
                }
                //执行保存方法
                setTimeout("DSE.app.doAjax()",500);
            },
            doAjax:function () {
                $ajax({
                    url: DSE.serverPath + 'wrmapatrolbCtrl/doSave.do',
                    async: true,
                    data: {
                        //ckID: row.ID,   //内容ID
                        taskID:DSE.app.ck_re_id,    //任务ID
                    },
                    type: 'post',
                    success: function(data) {
                        if(data["data"]) {
