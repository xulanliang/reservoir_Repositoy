package com.dse.reservoir.sjfx.spwy.web;
 ...
 */
@Controller
@RequestMapping("/spwyCtrl")
public class SpwyCtrl {

    @Resource(name = "spwyService")
    private SpwyService spwyService;
    ...
    public Object getDateList(HttpServletRequest request, HttpServletResponse response, EasyUiPage easyUiPage) throws Exception{
        try {
            ...
            return spwyService.getDateList(stm,etm,gcxArgs,easyUiPage);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Object getGcxDateList(HttpServletRequest request, HttpServletResponse response) throws Exception{
        try {
            ...
            List<Map> gcxdataList=(List<Map>)spwyService.getGcxDateList(stm,etm,gcxArgs);
            //按时间日期
             ...
             * 数据处理填充
             */
            SjfxDataUtil.execSjfxData(gcxdataList,gcxkeys,gcxvalues,dataValueMap,itemDataMap,"FSHIFT");
    ...
    public Object getDnpjtzzList(HttpServletRequest request, HttpServletResponse response, EasyUiPage easyUiPage) throws Exception{
        try {
            ...
            return spwyService.getDnpjtzzList(syear,eyear,projcd,easyUiPage);
        } catch (Exception e) {
            e.printStackTrace();
            return InvokeResult.failure(new InvokeResultMessage(e.getMessage()));
        }
    ...
    public Object getBaseDataList(HttpServletRequest request, HttpServletResponse response, EasyUiPage easyUiPage) throws Exception{
        try {
            ...
            String projcd=request.getParameter("projcd");
            easyUiPage.setPageSize(10000);
            return spwyService.getBaseDataList(syear,eyear,projcd,easyUiPage);
        } catch (Exception e) {
            e.printStackTrace();
            return InvokeResult.failure(new InvokeResultMessage(e.getMessage()));
        }
