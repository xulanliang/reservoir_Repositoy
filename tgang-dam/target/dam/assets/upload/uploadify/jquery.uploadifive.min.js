    ...
    var a = {
        init: function (c) {
            return this.each(function () {
                ...
                d.inputTemplate = b('<input type="file" accept="application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/pdf,application/msword,application/vnd.ms-excel">').css({
                ...
                d.addQueueItem = function (k) {
                    if (b.inArray("onAddQueueItem", f.overrideEvents) < 0) {
                        ...
                        k.queueItem.find('.file_index').html(d.fileID);
                        ...
                        k.queueItem.find(".fileName").val(m.split('.')[0]);
                        k.queueItem.find(".fileType").html(m.split('.')[1]);
                        k.queueItem.find(".fileSize").html(function(){
                            var _size;
                            if(k.size < 1024){
                                _size = k.size+'B';
                            }else if(k.size >= 1024 && k.size < 1024*1024){
                                _size = (k.size/1024).toFixed(2)+'KB';
                            }else if(k.size >= 1024*1024 && k.size < 1024*1024*1024){
                                _size = (k.size/(1024*1024)).toFixed(2)+'MB';
                            }
                            return _size;
                        });
                ...
                d.removeQueueItem = function (m, l, k) {
                    ...
                    var j = l ? 0 : 500;
                    if (m.queueItem) {
                        if (m.queueItem.find(".fileinfo").html() != "100%") {
                            m.queueItem.find(".fileinfo").html("0%")
                        }
                    ...
                    if ((e.uploads.count + e.uploads.current) < f.uploadLimit || f.uploadLimit == 0) {
                        ...
                        b("#" + f.queueID).find(".uploadifive-queue-item").not(".error, .complete").each(function () {
                            ...
                            $('#file_upload').data('uploadifive').settings.formData = {
                                'fileId': _file.lastModified,
                                'fileName': b(this).find('.fileName').val(),
                                'fileSecond': b(this).find('.fileSecond').val()
                            };
                            b(this).find(".fileState").html('上传中...');
                            b(this).find(".uploadify-progress-bar").animate({
                                width: '99%'
                            },_file.size/1024);
