var _dseDatagridTip = function($) {
                ...
                $(td).tooltip({
                    ...
                    onShow:function () {
                        ...
                        if(showParams.contentStyler){
                            tip.find('div.tipcontent').css(showParams.x);
                        }
                    }
                }).tooltip('show');
            };
    ...
    var showtips = function(id){
        $(id).datagrid('doCellTip', {
            onlyShowInterrupt : true,//默认false全显示，true只显示省略项;
            position : 'bottom',
            maxWidth : '250px',
            specialShowFields : [ {
                field : 'status',
...
if (typeof dseDefine === 'function') {
    dseDefine(function(require) {
        return _dseDatagridTip(require('easyui'));
    });
} else {
    // 依赖jquery
    _dseDatagridTip(jQuery);
}